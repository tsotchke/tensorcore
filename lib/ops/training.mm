...
extern "C" tc_status_t tc_fused_layernorm_gemv(tc_context* ctx,
                                               ...
                                               tc_buffer*       Y,
                                               int M, int N, int K, float eps) {
    if (!ctx || !X || !gamma || !beta || !W || !Y || M <= 0 || N <= 0 || K <= 0)
        return TC_ERR_INVALID_ARG;
    tc_status_t err = TC_OK;
    id<MTLComputePipelineState> pso = pso_for(ctx, @"tc_fused_layernorm_gemv_f16", &err);
    if (!pso) return err;
    ...
    @autoreleasepool {
        ...
        [enc setBuffer:gamma->mtl offset:0 atIndex:1];
        [enc setBuffer:beta->mtl  offset:0 atIndex:2];
        [enc setBuffer:W->mtl     offset:0 atIndex:3];
        [enc setBuffer:Y->mtl     offset:0 atIndex:4];
        [enc setBytes:&M_u length:sizeof(M_u) atIndex:5];
        [enc setBytes:&N_u length:sizeof(N_u) atIndex:6];
        [enc setBytes:&K_u length:sizeof(K_u) atIndex:7];
        [enc setBytes:&eps length:sizeof(eps) atIndex:8];
        const uint32_t T = threads_for_d(K_u);
        [enc dispatchThreadgroups:MTLSizeMake(N_u, M_u, 1)
            threadsPerThreadgroup:MTLSizeMake(T, 1, 1)];
