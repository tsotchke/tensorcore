#!/usr/bin/env python3
"""Validate JSON evidence from scripts/run_metal_ops_runtime_evidence.py."""
...
def load_json(path: pathlib.Path) -> Any:
    try:
        ...
        raise SystemExit(f"could not read Metal ops evidence {path}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise SystemExit(f"Metal ops evidence is not valid JSON: {exc}") from exc


def fail(errors: list[str]) -> int:
    print("Metal ops evidence invalid:", file=sys.stderr)
    for error in errors:
...
def check_required_functions(errors: list[str], data: dict[str, Any]) -> None:
    ...
    if missing:
        errors.append(f"Metal ops evidence is missing function coverage: {sorted(missing)!r}")
...
def check_clean_head(errors: list[str], data: dict[str, Any], expected_head: str | None) -> None:
    ...
    if meta.get("git_dirty") is not False:
        errors.append("Metal ops evidence must be from a clean git tree")
    if meta.get("git_head") != expected_head:
        errors.append(
            "Metal ops evidence git_head mismatch: "
...
def main() -> int:
    ...
    if not isinstance(data, dict):
        return fail(["Metal ops evidence root must be a JSON object"])
    meta = data.get("meta")
    ...
    if not isinstance(meta, dict) or meta.get("source") != "tensorcore_metal_ops_probe":
        errors.append("meta.source must be tensorcore_metal_ops_probe")
