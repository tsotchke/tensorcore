#!/usr/bin/env python3
"""Run quantized GEMV and GGUF smokes and emit ICC-readable evidence."""
...
TESTS = {
    "quantized": {
        "path": "tests/test_quantized",
        "required_markers": (
            "q4_0_gemv M=1 N=128 K=256",
            "q4_0_async M=1 N=128 K=256",
            "q8_0_gemv M=1 N=129 K=256",
            "fused_rmsnorm_q4_0_gemv",
            "fused_rmsnorm_q8_0_gemv",
            "invalid quant size: 0 OK",
        ),
        "covers": {
            "lib/ops/quantized.mm": {
                "gemv_quant_encode",
    ...
    "gguf": {
        "path": "tests/test_gguf",
        "required_markers": (
            "tensor       : name=weight.test",
            "gpu copy     : OK",
            "bulk load    : OK",
            "gguf gemv    : OK",
            "test_gguf: OK",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--evidence-path",
        type=pathlib.Path,
        default=ROOT / "build" / "quantized_gguf_runtime_evidence.json",
    )
...
def main() -> int:
    ...
    if args.json:
        ...
        print(
            "Quantized/GGUF runtime evidence "
