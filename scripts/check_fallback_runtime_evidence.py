#!/usr/bin/env python3
"""Validate evidence from scripts/run_fallback_runtime_smoke.py."""
...
def load_json(path: pathlib.Path) -> Any:
    try:
        ...
        raise SystemExit(f"could not read fallback evidence {path}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise SystemExit(f"fallback evidence is not valid JSON: {exc}") from exc


def fail(errors: list[str]) -> int:
    print("fallback runtime evidence invalid:", file=sys.stderr)
    for error in errors:
...
def check_clean_head(errors: list[str], data: dict[str, Any], expected_head: str | None) -> None:
    ...
    if meta.get("git_dirty") is not False:
        errors.append("fallback evidence must be from a clean git tree")
    if meta.get("git_head") != expected_head:
        errors.append(
            "fallback evidence git_head mismatch: "
...
def check_required_functions(errors: list[str], data: dict[str, Any]) -> None:
    ...
    if missing:
        errors.append(f"fallback runtime evidence is missing function coverage: {sorted(missing)!r}")
...
def main() -> int:
    ...
    if not isinstance(data, dict):
        return fail(["fallback evidence root must be a JSON object"])
    ...
    if not isinstance(meta, dict) or meta.get("source") != "tensorcore_fallback_runtime_smoke":
        errors.append("meta.source must be tensorcore_fallback_runtime_smoke")
    if data.get("status") not in ("passed", "failed"):
        errors.append(f"status must be passed or failed, got {data.get('status')!r}")
    ...
    if isinstance(checks, dict):
        for check in REQUIRED_CHECKS:
            ...
            if not isinstance(item, dict):
                errors.append(f"checks.{check} must be an object")
                continue
            if args.require_pass and item.get("status") != "passed":
                errors.append(f"checks.{check}.status must be passed, got {item.get('status')!r}")
            if item.get("status") == "passed" and item.get("label_seen") is not True:
                errors.append(f"checks.{check} passed without label_seen=true")
    ...
    covered = sum(len(names) for names in covered_functions(data).values())
    print(f"fallback runtime evidence OK: status={data.get('status')} covered_functions={covered}")
    return 0
