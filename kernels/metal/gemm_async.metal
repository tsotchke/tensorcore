...
/* Common kernel body templated on IO dtype. Apple7+ for half; Apple9+ for
 * bfloat (M3+). The math is identical — bf16 just has a different fp16-like
 * encoding. */
template <typename IO_T>
inline void tc_gemm_async_impl(
    device const IO_T*  A,
    device const IO_T*  B,
    device       IO_T*  C,
    ...
    uint2 group_id, uint sgid, uint slid,
    threadgroup IO_T* sA, threadgroup IO_T* sB,
    threadgroup float* scratch)
{
    ...
    for (uint kBlock = 0; kBlock < K; kBlock += ABK) {
        ...
        if (sgid == 0) {
            ...
            if (full_a) {
                ev[0].async_copy<IO_T>(
                    sA, ABK, ushort2(ABK, ABM),
                    A + baseRow * lda + kBlock, lda, ushort2(ABK, ABM));
            }
            if (full_b) {
                ev[1].async_copy<IO_T>(
                    sB, ABN, ushort2(ABN, ABK),
                    B + kBlock * ldb + baseCol, ldb, ushort2(ABN, ABK));
            }
            tc::simdgroup_event::wait(
                (full_a && full_b) ? 2 : (full_a || full_b) ? 1 : 0, ev);
        }
        ...
        if (!full_a || !full_b) {
            for (uint idx = tid; idx < ABM * ABK; idx += ATHREADS) {
                ...
                sA[row * ABK + col] = (gr < M && gc < K) ? A[gr * lda + gc] : IO_T(0);
            }
            for (uint idx = tid; idx < ABK * ABN; idx += ATHREADS) {
                ...
                sB[row * ABN + col] = (gr < K && gc < N) ? B[gr * ldb + gc] : IO_T(0);
            }
        }
        ...
        for (uint kk = 0; kk < ABK; kk += 8) {
            simdgroup_matrix<IO_T, 8, 8> a_frag[ATM];
            simdgroup_matrix<IO_T, 8, 8> b_frag[ATN];
            for (uint i = 0; i < ATM; ++i) {
...
/* Bf16 async kernel — Apple9+ (M3+). Same arithmetic as fp16 path,
 * bfloat encoding instead. Currently dispatched only when bf16
 * simdgroup_matrix is reported supported by the device. */
kernel void tc_gemm_bf16_f32_async(
...
{
    threadgroup bfloat sA[ABM * ABK];
    threadgroup bfloat sB[ABK * ABN];
    threadgroup float scratch[AWM * AWN * 64];
    tc_gemm_async_impl<bfloat>(
        A, B, C, M, N, K, alpha, beta, lda, ldb, ldc,
        group_id, sgid, slid, sA, sB, scratch);
}
