 ...
 * Each 4-bit weight w_i is dequantized as scale * (w_i - 8) where w_i ∈ [0, 15].
 ...
 *   - The 32 weights are packed two per byte: low nibble is weight 2*i,
 *     high nibble is weight 2*i+1 (little-endian within byte).
 *
...
/* Q4_0 GEMV: M batches × K (fp16 activations) @ K × N (Q4_0 weights) → M × N fp16.
    ...
    uint  lane                      [[thread_index_in_simdgroup]])
{
    ...
    for (uint b = 0; b < nblocks; ++b) {
        ...
        if (k < K) {
            const uchar packed = block[2 + (i / 2)];
            const uint nibble = (i & 1) ? (packed >> 4) : (packed & 0xF);
 ...
 * Higher fidelity than Q4_0; ~2× the size.
 *
    ...
    uint  lane                     [[thread_index_in_simdgroup]])
{
    ...
    if (lane < QK4 / 2) {
        const uint i_lo = lane * 2;
        const uint i_hi = lane * 2 + 1;
